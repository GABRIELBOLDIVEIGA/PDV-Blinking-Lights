...
export function Produto({ ...produto }: ItemPedidoValidator) {
  ...
  const addQuantidade = useProdutosStore((state) => state.addQuantidade);
  const updateDescontos = useProdutosStore((state) => state.updateDescontos);
  const resetProduto = useProdutosStore((state) => state.resetProduto);
  const handlePromocionalOuEspecial = useProdutosStore(
    (state) => state.handlePromocionalOuEspecial,
  );
  const handleCodigoDeBarras = useProdutosStore(
    (state) => state.handleCodigoDeBarras,
  );
  const removeProduto = useProdutosStore((state) => state.removeProduto);
  const updatePrecoEspecial = useProdutosStore(
