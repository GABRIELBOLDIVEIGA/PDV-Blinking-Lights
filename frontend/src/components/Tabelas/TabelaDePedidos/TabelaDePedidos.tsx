...
import { useTabelaPedidosStore } from "@/store/colunas-tabelas/useTabelaPedidosStore";
...
export function TabelaPedidosV2<TData, TValue>({
  data,
  columns,
  ...
  const { changeIsVisible } = useTabelaPedidosStore();
  const opt = useTabelaPedidosStore((state) => state.opt);
